...
public class FileService {
	...
	public void test2() {
		...
		// !붙으면 존재하지 않으면, 지정된 경로의 파일이 존재하지 않는다면
		if(!file.exists() ) {
			...
			if( file.createNewFile() ) {
				...
				System.out.println(file.getName() + "파일 생성 되었습니다.");
			}
		}catch(IOException e) {
			System.out.println("---- 파일 생성 중 예외발생 ----");
			
			e.printStackTrace(); 
			// 예외종류, 메시지, 예외가 발생한 위치출력
		}
